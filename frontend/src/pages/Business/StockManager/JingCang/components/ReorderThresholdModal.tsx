...
import type { JingCangStockInfoEntity } from 'services/generated/model';
...
interface ReorderThresholdModalProps {
  /** 是否可见 */
  visible: boolean;
  /** 库存信息 */
  stockInfo: JingCangStockInfoEntity | null;
  /** 表单引用 */
 ...
 * 设置补货预警阈值弹窗组件
 */
const ReorderThresholdModal: React.FC<ReorderThresholdModalProps> = ({
  visible,
  stockInfo,
  formRef,
  onClose,
  ...
  return (
    ...
    >
      <Form ref={formRef} labelWidth={120} colon>
        {stockInfo && (
          <>
            <FormItem label='所属库房'>{stockInfo.warehouse}</FormItem>
            <FormItem label='当前库存数量'>{stockInfo.stockQuantity}</FormItem>
            <FormItem
              label='补货预警阈值'
...
export default memo(ReorderThresholdModal);
